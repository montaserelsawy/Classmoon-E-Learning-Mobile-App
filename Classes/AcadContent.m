...
-(void)getDataJSON{
	NSString * JSONurl =[NSString stringWithFormat:@"%@?user_id=%@&module=academic&limit=%@",API_URL_LINK,[self getUserId],[self getAPIValue]];
	//connect to JSON
	NSArray *JSONArray = [self JSONConnect:JSONurl];
	//GET fields
	acad= [self JSONArray:JSONArray JSONField:@"attach_title"];
	publishDate= [self JSONArray:JSONArray JSONField:@"attach_created"];
	CourseIcon= [self JSONArray:JSONArray JSONField:@"course_icon"];
	CourseTitle= [self JSONArray:JSONArray JSONField:@"course_title"];
	Comments= [self JSONArray:JSONArray JSONField:@"attach_comments"];
	Reads= [self JSONArray:JSONArray JSONField:@"read"];
	NotifId= [self JSONArray:JSONArray JSONField:@"id"];
	FilePath = [self JSONArray:JSONArray JSONField:@"attach_original_filename"];
	AttachId = [self JSONArray:JSONArray JSONField:@"attach_id"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	topLabel.text = [acad objectAtIndex:indexPath.row];
	
	NSString * CommentBody = [NSString stringWithFormat:@"%@",[Comments objectAtIndex:indexPath.row]];
	bottomLabel.text = CommentBody;
	...
	//cell.textLabel.text = [acad	objectAtIndex:indexPath.row];
	//
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	AcadContentDetails * details = [[AcadContentDetails alloc] init];
	details.title = [acad objectAtIndex:indexPath.row];
	...
	details.FileName = [FilePath objectAtIndex:indexPath.row];
	details.AttachId = [AttachId objectAtIndex:indexPath.row];
		
	if ([details.Reads isEqualToString:@"0"]) {
		
		NSString * updateAsRead = [NSString stringWithFormat:@"%@?user_id=%@&module=academic&update=%@",API_URL_LINK,[self getUserId],details.NotifId];
